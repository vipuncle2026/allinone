...
router = APIRouter(prefix="/api/running", tags=["跑步"])

UPLOAD_DIR = get_upload_dir("running")
...
class ActivityCreate(BaseModel):
    ...
    running_route: Optional[str] = None
...
async def upload_activity_file(file: UploadFile = File(...)):
    ...
    # 从 start_time 推导 date（解析器通常不返回 date 字段，但 create 需要）
    if not data.get("date") and data.get("start_time"):
        try:
            dt = datetime.fromisoformat(data["start_time"].replace("Z", "+00:00").replace("+00:00", ""))
            data["date"] = dt.date()
        except Exception:
            pass
...
def create_activity(payload: ActivityCreate, db: Session = Depends(get_db)):
    """保存跑步活动（文件导入）"""
    act = RunningActivity(**payload.to_model_dict())
    db.add(act)
    db.commit()
    db.refresh(act)
    return act
...
def list_activities(
    ...
    """跑步活动列表（分页）"""
    q = db.query(RunningActivity)
    total = q.count()
    items = (
        q.order_by(RunningActivity.date.desc())
         .offset((page - 1) * limit)
         .limit(limit)
         .all()
    )
...
def get_stats(db: Session = Depends(get_db)):
    """跑步总览统计"""
    total = db.query(func.count(RunningActivity.id)).scalar()
    total_km = db.query(func.sum(RunningActivity.distance_km)).scalar() or 0
    total_sec = db.query(func.sum(RunningActivity.duration_sec)).scalar() or 0
    total_cal = db.query(func.sum(RunningActivity.calories)).scalar() or 0
    total_gain = db.query(func.sum(RunningActivity.elevation_gain)).scalar() or 0
    return {
...
def get_achievements(db: Session = Depends(get_db)):
    ...
    max_dist = (
        ...
        .order_by(RunningActivity.distance_km.desc().nullslast())
        .first()
    )
    ...
    longest = (
        db.query(RunningActivity)
        .order_by(RunningActivity.duration_sec.desc().nullslast())
        .first()
    )
    ...
    five_km = (
        db.query(RunningActivity)
        .filter(RunningActivity.distance_km >= 5)
        .order_by(RunningActivity.duration_sec.asc().nullslast())
        .first()
    )
    if five_km:
        speed = five_km.distance_km / (five_km.duration_sec / 3600) if five_km.duration_sec else 0
        if speed > 0:
            five_km_time = round(5 / speed * 3600)
            achievements["fastest_5km"] = {
                "value": five_km_time,
                "unit": "秒",
                "title": five_km.title,
                "date": str(five_km.date),
                "activity_id": five_km.id,
                "note": f"基于「{five_km.title}」({five_km.distance_km}km)配速推算",
            }

    # 10公里最快时间
    ten_km = (
        db.query(RunningActivity)
        .filter(RunningActivity.distance_km >= 10)
        .order_by(RunningActivity.duration_sec.asc().nullslast())
        .first()
    )
    if ten_km:
        ...
        if speed > 0:
            ten_km_time = round(10 / speed * 3600)
            achievements["fastest_10km"] = {
                ...
                "note": f"基于「{ten_km.title}」({ten_km.distance_km}km)配速推算",
            }

    # 半马最快（21.0975km）
    half = (
        db.query(RunningActivity)
        .filter(RunningActivity.distance_km >= 21.0975)
        .order_by(RunningActivity.duration_sec.asc().nullslast())
        .first()
    )
    if half:
        speed = half.distance_km / (half.duration_sec / 3600) if half.duration_sec else 0
        if speed > 0:
            half_time = round(21.0975 / speed * 3600)
            achievements["fastest_halfmarathon"] = {
                "value": half_time,
                "unit": "秒",
                "title": half.title,
                "date": str(half.date),
                "activity_id": half.id,
                "note": f"基于「{half.title}」({half.distance_km}km)配速推算",
            }

    # 全马最快（42.195km）
    full = (
        db.query(RunningActivity)
        .filter(RunningActivity.distance_km >= 42.195)
        .order_by(RunningActivity.duration_sec.asc().nullslast())
        .first()
    )
    if full:
        speed = full.distance_km / (full.duration_sec / 3600) if full.duration_sec else 0
        if speed > 0:
            full_time = round(42.195 / speed * 3600)
            achievements["fastest_marathon"] = {
                "value": full_time,
                "unit": "秒",
                "title": full.title,
                "date": str(full.date),
                "activity_id": full.id,
                "note": f"基于「{full.title}」({full.distance_km}km)配速推算",
            }
...
def year_summary(year: Optional[int] = None, db: Session = Depends(get_db)):
    ...
    q = db.query(RunningActivity).filter(
        extract('year', RunningActivity.date) == year
    )
    activities = q.order_by(RunningActivity.date.asc()).all()
    ...
    return {
        ...
        "avg_km_per_run": round(total_km / total_count, 1) if total_count else 0,
...
def get_activity(activity_id: int, db: Session = Depends(get_db)):
    """活动详情（含轨迹）"""
    act = db.query(RunningActivity).filter(RunningActivity.id == activity_id).first()
    if not act:
...
def _activity_to_dict(act: RunningActivity) -> dict:
    d = {}
    for c in RunningActivity.__table__.columns:
...
def export_backup(db: Session = Depends(get_db)):
    """导出全部跑步活动为 JSON 备份"""
    activities = [_activity_to_dict(a) for a in db.query(RunningActivity).order_by(RunningActivity.id).all()]
    payload = {
        "version": "1.0",
        "module": "running",
    ...
    buf.seek(0)
    filename = f"running_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
def export_csv(db: Session = Depends(get_db)):
    """导出跑步活动为 CSV 文件"""
    import csv
    activities = db.query(RunningActivity).order_by(RunningActivity.date.desc()).all()
    headers = [
        "ID", "活动名称", "日期", "距离(km)", "时长(秒)", "均速(km/h)",
        "最大速度(km/h)", "爬升(m)", "下降(m)", "最高海拔(m)", "最低海拔(m)",
        "配速(min/km)", "平均心率", "最大心率", "热量(kcal)",
        "平均步频", "最大步频", "步数", "跑步路线", "天气", "备注", "来源",
    ]
    rows = []
    for a in activities:
        rows.append([
            ...
            a.avg_cadence, a.max_cadence, a.steps, a.running_route, a.weather, a.notes, a.file_type,
        ])
    buf = io.StringIO()
    ...
    buf.seek(0)
    filename = f"running_activities_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
    return StreamingResponse(
